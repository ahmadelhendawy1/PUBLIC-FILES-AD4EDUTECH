...
function handleHTMLGeneration($body, $apiKey) {
    ...
    try {
        ...
        // ENHANCED VALIDATION: Check if HTML contains expected question counts
        ...
        // STRICT VALIDATION: If counts don't match, retry with stronger prompt
        $expectedMcqs = $sessionState['mcqCount'] ?? 0;
        $expectedGaps = $sessionState['gapCount'] ?? 0;
        
        if ($mcqMatches < $expectedMcqs || $gapMatches < $expectedGaps) {
            error_log("WARNING: Generated fewer questions than requested - retrying with enhanced prompt");
            
            // Retry with enhanced prompt
            $enhancedPrompt = buildEnhancedHTMLGenerationPrompt($sessionState, $language, $mcqMatches, $gapMatches);
            
            $retryPayload = [
                'model' => 'anthropic/claude-3.5-sonnet',
                'messages' => [
                    ['role' => 'system', 'content' => 'You are an expert HTML/CSS/JS developer. CRITICAL: Generate EXACTLY the specified number of questions. Count them before output. If you generate fewer questions than requested, you will be asked to regenerate.'],
            ...
            $retryCh = curl_init('https://openrouter.ai/api/v1/chat/completions');
            ...
            if ($retryHttpCode === 200) {
                ...
                if (isset($retryResult['choices'][0]['message']['content'])) {
                    $html = extractHTML($retryResult['choices'][0]['message']['content']);
                    
                    // Re-validate retry
                    $retryMcqMatches = substr_count($html, 'name="mcq-');
                    $retryGapMatches = substr_count($html, 'id="gap-');
                    
                    error_log("Retry validation - MCQs: {$retryMcqMatches}, Gaps: {$retryGapMatches}");
                }
            }
        }
...
function buildHTMLGenerationPrompt($state, $language) {
    ...
    // ENHANCED: Determine gap count based on content type
    $includeGaps = in_array($contentType, ['cloze', 'vocabulary', 'reading', 'mcq']);
    $gapCount = 0;
    
    if ($includeGaps) {
        // For vocabulary: 5-8 gaps
        if ($contentType === 'vocabulary') {
            $gapCount = min(8, max(5, $mcqCount));
        }
        // For reading: 3-6 gaps
        elseif ($contentType === 'reading') {
            $gapCount = min(6, max(3, $mcqCount));
        }
        // For cloze: 6-10 gaps
        elseif ($contentType === 'cloze') {
            $gapCount = min(10, max(6, $mcqCount));
        }
        // For MCQ: 4-6 gaps
        else {
            $gapCount = min(6, max(4, $mcqCount));
        }
    }
    
    // Update state with calculated gap count
...
function buildEnhancedHTMLGenerationPrompt($state, $language, $actualMcqs, $actualGaps) {
    $theme = $state['selectedTheme'] ?? 'light';
    $contentType = $state['contentType'] ?? 'mcq';
    $content = $state['content'] ?? '';
    $teacherName = $state['teacherName'] ?? 'Teacher';
...
- MCQs: $actualMcqs (needed: $mcqCount)
- Gaps: $actualGaps (needed: $gapCount)

**CRITICAL: You MUST generate EXACTLY the requested number of questions**

**REQUIREMENTS:**
- EXACTLY $mcqCount MCQ questions in Part A
- EXACTLY $gapCount Gap-fill questions in Part B
- Count each question as you write it
- If you don't reach the exact count, STOP and rewrite

**Source Content:**
$content

**Generate the complete HTML with:**
1. $mcqCount MCQs in the MCQS array
2. $gapCount gap-fill questions in the GAPS array
3. Working renderMcqs() and renderGaps() functions
4. All buttons functional
5. Certificate generation working
6. Print functionality working

**VERIFICATION STEPS:**
1. Count MCQS array items - must be $mcqCount
2. Count GAPS array items - must be $gapCount
3. Ensure both tabs are present
4. Ensure all functions are implemented
